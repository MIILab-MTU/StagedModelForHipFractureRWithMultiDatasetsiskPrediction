...
for (age_label in names(age_groups)) {
  ...
  for (race in race_levels) {
    ...
    # Apply T-score transformation for matching individuals in MrOS_clinical
...
dim(MrOS_clinical)
ha_sof <- colnames(MrOS_clinical)[grepl("^HA_", colnames(MrOS_clinical))]
MrOS_clinical_subset <- MrOS_clinical[, ha_sof]
MrOS_clinical_subset1 <- MrOS_clinical_subset[, -8]
# Remove same varaibles "HA_PHYS"+"HA_IADL"="HA_IADL51"
MrOS_clinical_subset1 <- MrOS_clinical_subset1[, !names(MrOS_clinical_subset1) %in% c("HA_PHYS", "HA_IADL")]
dim(MrOS_clinical_subset1)
colnames(MrOS_clinical_subset1)
MrOS_clinical2 <- MrOS_clinical_subset1
MrOS_clinical2 <- MrOS_clinical_subset1[!is.na(MrOS_clinical_subset1$HA_HIPFX), ]
## if missing greater than 20%, remove those variables
MrOS_clinical21 <- MrOS_clinical2[, colSums(is.na(MrOS_clinical2)) <= 720] 
dim(MrOS_clinical21)
str(MrOS_clinical21)

MrOS_clinical21 <- MrOS_clinical21 %>%
  mutate(
...
#stage1 <- MrOS_clinical21[, !names(MrOS_clinical21) %in% c("HA_THD", "HA_LSD", "HA_FND","HA_HIPFXFU","HA_WRSTFX", "HA_SLDFX","HA_WRSTFXFU","HA_SLDFXFU","HA_CALCIUM","HA_CROFTOA","HA_VITAL")]
#stage <- stage1
stage2 <- MrOS_clinical21[, !names(MrOS_clinical21) %in% c("HA_HIPFXFU","HA_WRSTFXFU","HA_SLDFXFU","HA_VITAL")]
stage <- stage2

table(stage$HA_RACE,stage$HA_HIPFX)
# Compute row-wise proportions
# Compute row-wise proportions and round to 3 decimals
prop_table <- round(prop.table(table(stage$HA_RACE, stage$HA_HIPFX), margin = 1), 3)
...
HA_HIPFX <- MrOS_clinical21$HA_HIPFX

# Combine transformed numerical data with categorical variables
dfs_final <- cbind(dfs_spatial_sign, dfs_categorical,HA_HIPFX)
...
#optVariables <- head(rfe_result$optVariables,5)
## feature selection
ensemble2.optVariables <- c("HA_FND", "HA_GRIPAVG", "HA_THD", "HA_WLKSPED", "HA_LSD", "HA_BMI",
  ...
  "HA_SMOKE", "HA_KIDNYST")
optVariables <- ensemble2.optVariables

#ensemble1.optVariables <- c("HA_HEIGHT","HA_GRIPAVG","HA_IADL51","HA_WLKSPED","HA_BMI")

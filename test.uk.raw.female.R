...
df <- MrOS_clinical[, c("B1THD", "B1TLD", "B1FND", "HA_GRIPAVG","walk","HA_SLDFX","HA_WRSTFX",
...
dxa_features <- c("B1THD", "B1TLD", "B1FND")
target_col <- "HA_HIPFX"
...
df <- SOF_clinical[, c("HTOTBMD", "STOTBMD", "NBMD",  "HA_GRIPAVG","walk","HA_SLDFX","HA_WRSTFX",
                       "HA_HEIGHT", "HA_AGE", "HA_WEIGHT","HA_IADL51","HA_SMOKE", "HA_HIPFX")]
...
dxa_features <- c("HTOTBMD", "STOTBMD", "NBMD")
target_col <- "HA_HIPFX"

# Drop rows with missing values in selected features
# Create a vector of columns to check for NA values
uk_data <- read.csv("/Users/mac/Desktop/dissertation/cooperation/FOS/UK_raw1200.dataset.csv", header = TRUE, stringsAsFactors = FALSE)
...
testUK_dxa <- as.data.frame(cbind(UK$newtotalbmd,UK$spinebmd,UK$newneck))
colnames(testUK_clinical) <- c( "HA_GRIPAVG", "walk" , "HA_SLDFX" ,  "HA_WRSTFX" , "HA_HEIGHT", "HA_AGE", "HA_WEIGHT" ,"HA_IADL51","HA_SMOKE")
#colnames(testUK_dxa) <- c("B1THD", "B1TLD", "B1FND")
colnames(testUK_dxa) <- c("HTOTBMD", "STOTBMD", "NBMD")
